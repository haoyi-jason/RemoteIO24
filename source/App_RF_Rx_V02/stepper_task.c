...
#define STEPPER_PUL_LINE        PAL_LINE(GPIOC,6)
#define STEPPER_DIR_LINE        PAL_LINE(GPIOC,7)
...
#define STEPPER_POLL_TIMER      &GPTD8
...
//        gptStartOneShot(STEPPER_POLL_TIMER,1000);
        ...
        if(reload == -1){
          if(!inIdle){
            ...
            if(TIME_I2MS(chVTTimeElapsedSinceX(idleStart))>1000){
              hfc_dis();
            }
          }
        }
